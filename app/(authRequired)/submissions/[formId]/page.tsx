...
export default async function Page({ params }: { params: { formId: string } }) {
  ...
  return (
    <div>
      ...
      <div className="pb-20">
        <div className=" hidden lg:flex mt-8 border-b-[1px] items-center px-56 pb-8 ">
          <p className="font-bold text-4xl mt-auto">{form.data?.name}</p>
          <Link href={`/form/${params.formId}`} className="ml-auto">
            <Button className=" scale-90 px-12 text-md font-medium">
              Visit
            </Button>
          </Link>
        </div>
        <div className="lg:flex hidden mt-8 border-b-[1px] items-center px-56 pb-8">
          <input
            type="text"
            readOnly
            value={`https://form-builder-one.vercel.app/form/${params.formId}`}
            className="focus-within:outline-none focus-within:border-2 focus-within:border-foreground/30 pl-2 bg-background border-[1px] rounded-sm w-full py-1"
          />
          <CopyBtn
            formLink={`https://form-builder-one.vercel.app/form/${params.formId}`}
          />
        </div>
        <div className="flex  flex-row lg:flex-nowrap flex-wrap w-full gap-8 lg:px-56 mt-10  ">
          <StatsCard
            ...
            color="#2563EB"
            statistic={form.data?.views.toString() ?? "0"}
          />
          <StatsCard
            ...
            statistic={form.data?.subCount.toString() ?? "0"}
          />
          <StatsCard
            icon="subRate.svg"
            ...
            color="rgb(34 197 94)"
            statistic={calcSubRate(form.data?.views, form.data?.subCount)}
          />
          <StatsCard
            icon="bounce.svg"
            title="Bounce rate"
            ...
            color="rgb(239 68 68)"
            statistic={calcBounceRate(form.data?.views, form.data?.subCount)}
          />
        </div>
        <div className="lg:px-56 ">
          ...
          <div className="w-full lg:px-0 px-1  overflow-x-scroll   mt-5 ">
            ...
            {answers.map((data, index) => (
              <div
                className=" hover:bg-accent/10 hover:cursor-pointer flex pl-4 border-b-[1px] h-10 items-center text-sm text-foreground/60 font-semibold gap-3 w-full"
                key={index}
              >
